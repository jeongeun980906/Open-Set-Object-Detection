...
def find_top_rpn_proposals_uncertainty(
    proposals: List[torch.Tensor], uncertainty:Dict,
    pred_objectness_logits: List[torch.Tensor],
    image_sizes: List[Tuple[int, int]],
    nms_thresh: float,
    pre_nms_topk: int,
    ...
    num_images = len(image_sizes)
    device = proposals[0].device
    aleas = uncertainty['alea'] # [N x D]
    episs = uncertainty['epis'] # [N x D]
    ...
    for level_id, (proposals_i, logits_i) in enumerate(zip(proposals, pred_objectness_logits)):
        ...
        topk_scores.append(topk_scores_i)
        level_ids.append(torch.full((num_proposals_i,), level_id, dtype=torch.int64, device=device))
        topk_alea.append(aleas[batch_idx[:, None],topk_idx])
        topk_epis.append(episs[batch_idx[:, None],topk_idx])

    # 2. Concat all levels together
    topk_scores = cat(topk_scores, dim=1)
    topk_proposals = cat(topk_proposals, dim=1)
    level_ids = cat(level_ids, dim=0)
    topk_alea = cat(topk_alea,dim=1)
    topk_epis = cat(topk_epis,dim=1)
